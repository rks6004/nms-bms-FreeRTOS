...
void printVoltages(uint8_t tIC, uint8_t t2950IC, cell_asic_6830 *IC, TYPE type)
{
  ...
  // Not reading first IC (represents ADBMS2950) 
  // Maybe could add an additional parameter to the function for software to adapt 
  // to more pack monitors for future (replace 1 with parameter)
  for(uint8_t ic = t2950IC; ic < tIC; ic++)
  {
    // Added the -1 (although redundant) because for the same reason above the (- 1) can be replaced
    // by (- ic2950_param) to subtract the # of ic's we're ignoring due to ADBMS2950 being placed
    // at the beginning of the chain
    printf("IC%d:",(ic + 1 - t2950IC));
...
/**
 * @brief Print cell temperatures
 * 
 * @param [in]	tIC      Total IC
 * @param [in]  offset   Any number of ICs that may need to not be displayed
 * @param [in]  IC      cell_asic_6830 stucture pointer
 */
void printCellTemperatures(uint8_t tIC, uint8_t offset, cell_asic_6830 *IC)
{
  for(int current_ic = offset; current_ic < tIC; current_ic ++)
  {
    printf("IC%d:",(current_ic + 1 - offset));
    for(int temp = 0; temp < (AUX - 2); temp ++)
    {
      printf("TEMP%d=%f C,", (temp + 1), IC[current_ic].cell_temperatures.cell_temps[temp]);
    }
    printf("\n\n");
  }
}
